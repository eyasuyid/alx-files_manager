...
class FilesController {
  static async postUpload(req, res) {
    ...
    if (parentId) {
      ...
      const parent = await db.files.findOne({ _id: new ObjectId(parentId) });
  ...
  static async getShow(req, res) {
    ...
    const requestedFile = await db.files.findOne({
      _id: ObjectId(id),
  ...
  static async getIndex(req, res) {
    ...
    const requestedFiles = await db.files.aggregate(query).toArray();
  ...
  static async getFile(req, res) {
    ...
    const requestedFile = await db.files.findOne({ _id: ObjectId(id) });
    if (!requestedFile) return res.status(404).send({ error: 'Not found' });
